...
- Continuity: Gaps in numbering are permitted (e.g., `1` and `3` without `2`). A parent task must exist before adding a child task.

### Task States

- Independence: State management is independent; updating a parent does not affect children. Marking a task as complete or incomplete applies only to that specific task.
- Mutations: Adding or replacing a task sets it to incomplete (☐).
- Deletion: Deleting a task removes that task and all its descendants.
