...
deterministic, fixed-ID architecture, `plan` reduces the cognitive load on the
agent. It provides a stable, queryable source of truth, allowing agents to
reliably execute, track, and modify workflows without context degradation.

## Built for Agents

- **Stable Addressing:** Tasks are referenced by absolute, dot-separated IDs
  (e.g., `1.2.1`). Because node ids never change, the agent is guaranteed to
  operate on the exact node it intends to, even as the tree evolves.
...
- **Atomic Resilience:** Batch operations are transactional. If an agent
  ...
  `plan` aborts the transaction and returns a standard error, preventing
...
```bash
agent@serenity:~$ plan 1.1 "Install PostgreSQL" 1.2 "Create user and schema" && plan 1
...
agent@serenity:~$ plan complete 1.1.1 && plan complete 1.1 && plan 1
☐ 1 "Provision Database"
☒ 1.1 "Install PostgreSQL"
...
```bash
agent@serenity:~$ plan complete 1 && plan 2
☐ 2 "Setup Backend API"
```
